...
class YOLOTrainingManager(QObject):
    ...
    def analyze_dataset(
        self, dataset_path: str, task: str = "detect"
    ) -> Dict[str, Any]:
        """
        Анализирует датасет
        ...
        **augmentation_params,
        ...
        try:
                    ...
                    try:
                        ...
                        if "error" in result:
                            self.manager.training_completed.emit(
                                self.experiment_id, False, result["error"]
                            )
                        else:
                        ...
                        self.manager.training_completed.emit(
                            self.experiment_id, False, str(e)
                        )
            ...
            thread = TrainingThread(
                self.detection_trainer, train_params, self, experiment_id
            )
    ...
    def start_segmentation_training(
        self,
        dataset_path: str,
        ...
        **augmentation_params,
        ...
        try:
            ...
            # Создаем и запускаем поток (передаем experiment_id отдельно)
            thread = TrainingThread(
                self.segmentation_trainer, train_params, self, experiment_id
            )
            self.active_trainings[experiment_id] = thread
            thread.start()
    ...
    def get_experiment_summary(self, experiment_id: str) -> Dict:
        ...
        try:
            ...
            # Получаем метрики
            metrics = self.metrics_tracker.database.get_experiment_metrics(
                experiment_id
            )
