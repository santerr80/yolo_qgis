...
class DatasetManager:
                ...
                if os.path.exists(split_dir):
                    for file in os.listdir(split_dir):
                        if file.lower().endswith((".png", ".jpg", ".jpeg")):
                            self.existing_images.add(
                                os.path.join("images", split, file).replace("\\", "/")
                            )
        ...
        for line in lines:
            if line.startswith("names:"):
                ...
                for class_id, class_name in sorted(
                    self.class_names.items(), key=lambda x: int(x[0])
                ):
    ...
    def append_new_data(
        ...
        try:
            ...
            if self.dataset_type == "ndjson":
                return self._append_ndjson_data(
                    intersected_layer,
                    grid_layer,
                    class_field,
                    image_format,
