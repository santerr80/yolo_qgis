...
class SegmentationDatasetAnalyzer:
    """Анализатор датасета для сегментации"""

    @staticmethod
    def analyze(dataset_path: str) -> Dict[str, Any]:
        """
        Анализирует датасет для сегментации
        ...
        try:
            ...
            if os.path.exists(yaml_path):
                ...
                result = SegmentationDatasetAnalyzer._analyze_yolo_format(
                    dataset_path, yaml_path
                )
            elif os.path.exists(ndjson_path):
                # NDJSON формат
                result = SegmentationDatasetAnalyzer._analyze_ndjson_format(
                    dataset_path, ndjson_path
                )
            else:
                        ...
                        with open(label_file, "r") as f:
                            ...
                            for line in lines:
                                ...
                                if parts:
                                    ...
                                    # Для сегментации считаем количество точек в сегменте
                                    if len(parts) > 5:  # Есть координаты сегмента
                                        total_segments += 1
            ...
            for split, records in splits_data.items():
                ...
                for record in records:
                    ...
                    for ann in annotations:
                        ...
                        if "segments" in ann or "polygon" in ann:
                            total_segments += 1

                result["splits"][split] = {
                    "image_count": len(records),
                    "annotation_count": len(
                        [r for r in records if r.get("annotations")]
                    ),
                    "total_objects": total_objects,
                    "total_segments": total_segments,
...
class SegmentationTrainer:
    """Тренер для моделей сегментации YOLO"""
    ...
    def train(
        ...
        try:
            ...
            if not os.path.exists(yaml_path):
                ...
                if not yaml_path:
                    return {
                        "error": "Не найден dataset.yaml и не удалось создать из NDJSON"
                    }
            ...
            if progress_callback:
                ...
                def on_fit_epoch_end(trainer):
                    ...
                    try:
                                    ...
                                    try:
                                        if hasattr(metrics_dict, attr_name):
                                            ...
                                            if value is not None:
                                                training_metrics[attr_name] = float(
                                                    value
                                                )
                        ...
                        try:
                            logger.error(
                                f"Ошибка в callback прогресса: {e}", exc_info=False
                            )
                        except:
                ...
                try:
                    self.current_model.add_callback(
