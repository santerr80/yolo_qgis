# -*- coding: utf-8 -*-
"""
Тренер для обучения моделей сегментации объектов YOLOv8-seg и YOLOv11-seg
...
class SegmentationTrainer:
    """Тренер для моделей сегментации объектов"""
    
    def __init__(self, model_type: str = "yolov8n-seg", device: str = "cpu"):
        """Инициализация тренера
        
        Args:
            model_type: Тип модели (yolov8n-seg, yolov8s-seg, yolov8m-seg, yolov8l-seg, yolov8x-seg,
    ...
    def train(self, dataset_path: str, epochs: int = 100, batch_size: int = 16,
        ...
        log_message(f"Начало обучения модели сегментации: {self.model_type}")
        ...
        train_params = {
            ...
            'lr0': learning_rate,
            'device': self.device,
            'project': save_dir or "runs/segment",
            'name': project_name,
        ...
        if augmentation_params:
            # Стандартные параметры аугментации ultralytics для сегментации
        ...
        try:
            ...
            log_message(f"Директория сохранения: {save_dir or 'runs/segment'}/{project_name}")
                        ...
                        try:
                            ...
                            if hasattr(trainer, 'metrics') and trainer.metrics:
                                ...
                                if isinstance(metrics_dict, dict):
                                    # Для сегментации используем метрики с суффиксом (M)
                                    metrics['mAP50'] = float(metrics_dict.get('metrics/mAP50(M)', 0.0))
                                    metrics['mAP50-95'] = float(metrics_dict.get('metrics/mAP50-95(M)', 0.0))
                                    metrics['precision'] = float(metrics_dict.get('metrics/precision(M)', 0.0))
                                    metrics['recall'] = float(metrics_dict.get('metrics/recall(M)', 0.0))
                            ...
                            if not metrics and hasattr(trainer, 'validator') and trainer.validator:
                                if hasattr(trainer.validator, 'metrics'):
                                    ...
                                    if isinstance(val_metrics, dict):
                                        metrics['mAP50'] = float(val_metrics.get('metrics/mAP50(M)', 0.0))
                                        metrics['mAP50-95'] = float(val_metrics.get('metrics/mAP50-95(M)', 0.0))
                                        metrics['precision'] = float(val_metrics.get('metrics/precision(M)', 0.0))
                                        metrics['recall'] = float(val_metrics.get('metrics/recall(M)', 0.0))
...
class SegmentationDatasetAnalyzer:
    """Анализатор датасетов для сегментации"""
    ...
    def analyze(dataset_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                dataset_config = SegmentationDatasetAnalyzer._parse_yaml_simple(dataset_yaml)
            else:
            ...
            for split_name in ['train', 'val', 'test']:
                ...
                if split_key in dataset_config:
                    ...
                    split_analysis = SegmentationDatasetAnalyzer._analyze_split(
    ...
    def _analyze_split(split_path: str, split_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Для сегментации аннотации содержат полигоны масок
            ...
            if os.path.exists(labels_path):
                ...
                # Анализируем аннотации (для сегментации формат тот же - YOLO)
                    ...
                    try:
                        with open(txt_file, 'r') as f:
                            for line in f:
                                ...
                                if line:
                                    parts = line.split()
                                    if len(parts) >= 5:  # class_id + минимум 2 точки для полигона
