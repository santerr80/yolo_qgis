...
class DatasetManagerDialog(QtWidgets.QDialog):
    ...
    def on_datasets_found(self, datasets):
        ...
        if datasets:
            QMessageBox.information(
                self, "Результат", f"Найдено датасетов: {len(datasets)}"
            )
        else:
    ...
    def show_dataset_info(self):
        ...
        try:
            ...
            info_dialog.setWindowTitle(
                f"Информация о датасете: {os.path.basename(dataset_path)}"
            )
    ...
    def create_backup(self):
        ...
        try:
            ...
            if backup_dir:
                success, message = DatasetUtils.create_dataset_backup(
                    dataset_path, backup_dir
                )
                if success:
            ...
            QMessageBox.critical(
                self, "Ошибка", f"Ошибка создания резервной копии: {e}"
            )
    ...
    def export_dataset_info(self):
        ...
        try:
            ...
            if output_file:
                success, message = DatasetUtils.export_dataset_info(
    ...
    def open_dataset(self):
        ...
        try:
            ...
            if not os.path.exists(dataset_path):
                QMessageBox.warning(
                    self, "Ошибка", f"Директория не существует: {dataset_path}"
                )
                return
                    ...
                    try:
                        ...
                        if result.returncode != 0:
                            ...
                            QMessageBox.warning(
                                self,
                                "Ошибка",
                                f"Не удалось открыть директорию. Код ошибки: {result.returncode}",
                            )
                        ...
                        QMessageBox.warning(
                            self,
                            "Ошибка",
                            f"Не удалось открыть директорию: {e_subprocess}",
                        )
            ...
            if reply == QMessageBox.Yes:
                try:
                    ...
                    QMessageBox.information(
                        self, "Успех", "Путь скопирован в буфер обмена"
                    )
                except Exception as e_clipboard:
                    QMessageBox.warning(
                        self,
                        "Ошибка",
                        f"Не удалось скопировать в буфер обмена: {e_clipboard}",
                    )
