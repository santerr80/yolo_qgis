...
def save_yolo_native_dataset(
    ...
    try:
        ...
        for i, tile_feature in enumerate(grid_layer.getFeatures()):
            ...
            if not features_in_tile and delete_void:
                if os.path.exists(source_image_path):
                    try:
                        ...
                        print(f"Ошибка при удалении пустого тайла {source_image_path}: {e}")
                continue
            ...
            for feat in features_in_tile:
                ...
                if task_type == 'detect':
                    ...
                    annotations.append(f"{class_id} {box_center_x:.6f} {box_center_y:.6f} {box_width:.6f} {box_height:.6f}")

                elif task_type == 'segment':
                    if geom.wkbType() not in [QgsWkbTypes.Polygon, QgsWkbTypes.MultiPolygon]: continue
        ...
        try:
            ...
            print(f"Предупреждение: не удалось удалить временную директорию {exported_images_dir}.")

        return True, None
    except Exception as e:
        return False, f"Произошла критическая ошибка при форматировании нативного датасета YOLO: {e}"