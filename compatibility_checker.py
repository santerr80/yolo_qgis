...
class CompatibilityChecker:
    ...
    def check_numpy_compatibility(self) -> Tuple[bool, str]:
        ...
        try:
            ...
            if major_version >= 2:
                return (
                    False,
                    f"numpy версии {np.__version__} несовместима с QGIS. Требуется numpy v1.x",
                )

            if major_version == 1 and minor_version < 21:
                return (
                    False,
                    f"numpy версии {np.__version__} слишком старая. Требуется >= 1.21.0",
                )
    ...
    def _check_version_compatibility(
        self, package: str, installed_version: str, version_req: str
    ) -> Tuple[bool, str]:
        """
        ...
        try:
            ...
            if package == "numpy":
                ...
                if major_version >= 2:
                    return (
                        False,
                        f"numpy {installed_version} несовместима с QGIS (требуется v1.x)",
                    )
                    ...
                    if minor_version < 21:
                        return (
                            False,
                            f"numpy {installed_version} слишком старая (требуется >= 1.21.0)",
                        )
                    else:
            ...
            if ">=1.0.0,<2.0.0" in version_req:
                if major_version >= 2:
                    return (
                        False,
                        f"{package} {installed_version} может быть несовместима (требуется v1.x)",
                    )
                else:
    ...
    def check_qgis_compatibility(self) -> Tuple[bool, str]:
        ...
        try:
            ...
            return (
                True,
                f"QGIS совместим (версия: {QgsApplication.instance().applicationVersion()})",
            )
        except ImportError:
