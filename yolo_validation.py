...
class AdvancedValidator:
    ...
    def validate(self, model_path: str, dataset_path: str, task: str = 'detect',
                ...
                max_detections: int = 300, comprehensive: bool = True,
        ...
        try:
            ...
            if hasattr(results, 'results_dict'):
                ...
                metrics = {
                    ...
                    'f1_score': 2 * (getattr(results.box, 'mp', 0.0) * getattr(results.box, 'mr', 0.0)) / 
                               (getattr(results.box, 'mp', 0.0) + getattr(results.box, 'mr', 0.0)) 
                               if (getattr(results.box, 'mp', 0.0) + getattr(results.box, 'mr', 0.0)) > 0 else 0.0
                }
                ...
                metrics = {
                    ...
                    'f1_score': 2 * (getattr(results.seg, 'mp', 0.0) * getattr(results.seg, 'mr', 0.0)) / 
                               (getattr(results.seg, 'mp', 0.0) + getattr(results.seg, 'mr', 0.0)) 
                               if (getattr(results.seg, 'mp', 0.0) + getattr(results.seg, 'mr', 0.0)) > 0 else 0.0
                }
    ...
    def _comprehensive_validation(self, model, dataset_path: str, task: str,
    ...
    def _get_class_metrics(self, model, dataset_path: str, task: str, device: str) -> Dict[str, Any]:
            ...
            if hasattr(results, 'names'):
                for class_id, class_name in class_names.items():
                    class_metrics[class_name] = {
                        'precision': getattr(results, f'p{class_id}', 0.0) if hasattr(results, f'p{class_id}') else 0.0,
                        'recall': getattr(results, f'r{class_id}', 0.0) if hasattr(results, f'r{class_id}') else 0.0,
                        'mAP50': getattr(results, f'map50{class_id}', 0.0) if hasattr(results, f'map50{class_id}') else 0.0
                    }
    ...
    def _get_size_metrics(self, model, dataset_path: str, task: str, device: str) -> Dict[str, Any]:
    ...
    def _analyze_errors(self, model, dataset_path: str, task: str, device: str) -> Dict[str, Any]:
